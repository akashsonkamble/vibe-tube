...
const Sidebar = () => {
    ...
    return (
        <>
            ...
            <aside 
                className={`w-56 absolute top-0 overflow-y-auto scrollbar-hidden pb-4 flex-col gap-2 px-2 lg:sticky ${isLargeOpen ? "lg:flex" : "lg:hidden"} ${isSmallOpen ? "flex z-50 bg-white max-h-screen" : "hidden"}`}
            >
                ...
                <LargeSidebarSection visibleItemCount={5}>
                    ...
                    <LargeSidebarItem IconOrImgUrl={Clock} title="Watch Later" url="/playlists/?list=WL" />
                    {playlists.map(({ id, name }: { id: string; name: string; }) => (
                        <LargeSidebarItem
                            key={id}
...
const LargeSidebarSection = ({ 
    ...
    const visibileChildren = isExpanded ? childrenArray : childrenArray.slice(0, visibleItemCount);
...
const LargeSidebarItem = ({ IconOrImgUrl, title, url, isActive = false }: LargeSidebarItemProps) => {
    return <a href={url} className={`twMerge(buttonStyles({ variant: "ghost" }), "w-full flex items-center rounded-lg gap-4 p-3 ${isActive ? "font-bold bg-neutral-100 hover:bg-secondary" : undefined}`}>
        {typeof IconOrImgUrl === "string" ? 
            <img src={IconOrImgUrl} alt={title} className="w-6 h-6 rounded-full" /> : <IconOrImgUrl className="w-6 h-6" />
        }
        <div className="whitespace-nowrap overflow-hidden text-ellipsis">
            {title}
        </div>
